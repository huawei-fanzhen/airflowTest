...
from dag_action.action import addDagService
...
apis_metadata = [
    ...
    {
        "name": "add_dag",
        "description": "add a new DAG File to the DAGs directory",
        "airflow_version": "None - Custom API",
        "http_method": ["GET", "POST"],
        "arguments": [
            {"name": "dag_name", "description": "The name of the dag", "form_input_type": "text", "required": True}
        ]
    },
    {
        "name": "refresh_dag",
...
class REST_API(get_baseview()):
    ...
    def api(self):
        ...
        if api == "version":
            ...
            final_response = self.add_dag(base_response)
        else:
    ...
    def add_dag(self,base_response):
        dag_name = request.args.get('dag_name')
        try:
            addDagService(dag_name)
        except Exception as e:
            error_message = "An error occurred while trying to add the DAG '" + str(dag_name) + "': " + str(e)
            logging.error(error_message)
            return REST_API_Response_Util.get_500_error_response(base_response, error_message)
        return REST_API_Response_Util.get_200_response(base_response=base_response, output="DAG [{}] is now add into dag directory".format(dag_name))
